 ...
 */
import Inspector, { getBoolean } from './swiper-inspector';

const { Component, Fragment } = wp.element;
const { InnerBlocks } = wp.editor;
const { swiperParamsFromDataSet } = window.axisTaylor;

const ALLOWED_BLOCKS = [ 'axistaylor-guten/swiper-slide' ];

const getSliderTemplate = memoize( ( slides ) => {
	return times( slides, () => [ 'axistaylor-guten/swiper-slide' ] );
} );
 ...
 */
export const save = ( { attributes: { id, params: { 0: params }, align }, className, customClassName } ) => (
	<div
		id={ id }
		className={
			classNames(
